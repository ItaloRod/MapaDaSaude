...
$sql = "SELECT DISTINCT cbo_descricao FROM $schemaCnes.elastic_sagu_cnes_egressos A WHERE cbo_descricao <> ''";
$query1 = $conSagu->query($sql);
while ($row = $query1->fetch(PDO::FETCH_OBJ)) {
    $sqlInsert5 = "INSERT INTO public.term (taxonomy, term, description) 
                    VALUES ('tag', '" . $row->cbo_descricao . "', '" . $row->cbo_descricao . "')";
...
$idAgentTmp = null;
$sql = "SELECT DISTINCT A.personid, B.email FROM $schemaCnes.elastic_sagu_cnes_egressos A
...
while ($rowProfissional = $query1->fetch(PDO::FETCH_OBJ)) {
    ...
    $sql = "
        SELECT DISTINCT pessoa,
           ...
           FROM cnes12_2019.elastic_sagu_cnes_egressos a
    ...
    while ($row = $query2->fetch(PDO::FETCH_OBJ)) {
        ...
        if ($row->longitude != null) {
            ...
            $descricao = 'Egresso - Escola de Saúde Pública do Ceará | ';
            ...
            $sqlInsertSeal = "INSERT INTO public.seal_relation 
                    ...
                    VALUES ('1', '" . $idAgent . "', '" . $dataHora . "' , '1' , 'MapasCulturais\Entities\Agent' , '1' ,'" . $idAgent . "' ,
